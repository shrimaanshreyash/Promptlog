...
export function addNote(promptId: string, options: NoteOptions) {
  const db = getDb();

  const prompt = db.prepare('SELECT * FROM prompts WHERE stable_name = ? OR id = ?').get(promptId, promptId) as any;
  if (!prompt) {
  ...
  if (options.version) {
    ...
    const version = db.prepare('SELECT * FROM prompt_versions WHERE prompt_id = ? AND version_number = ?').get(prompt.id, versionNum) as any;
    if (!version) {
  ...
  `).run(
    ...
    prompt.id,
    version_id,
    options.type || 'general_note',
    options.title || 'CLI Note',
    options.body || '',
...
export function listNotes(promptId: string, options: { version?: string; type?: string; severity?: string }) {
  const db = getDb();
  ...
  for (const note of notes) {
    ...
    if (note.title) console.log(`║       ${note.title.substring(0, 42)}`);
    if (note.body) console.log(`║       ${note.body.substring(0, 42)}`);
...
export function deleteNote(noteId: string) {
  ...
  const note = db.prepare('SELECT * FROM prompt_notes WHERE id = ? OR id LIKE ?').get(noteId, `${noteId}%`) as any;
  if (!note) {
