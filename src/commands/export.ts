...
export function exportData(projectRoot: string, config: PromptLogConfig, options: any) {
  const db = getDb();
...
function exportJson(projectRoot: string, config: PromptLogConfig, db: any) {
...
function exportMarkdown(projectRoot: string, config: PromptLogConfig, db: any) {
  ...
  for (const prompt of prompts) {
    ...
    for (const v of versions) {
      ...
      const vNotes = notes.filter((n: any) => n.version_id === v.id);
      if (vNotes.length > 0) {
        md += `**Notes:**\n\n`;
