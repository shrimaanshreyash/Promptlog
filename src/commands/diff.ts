...
export function showDiff(promptId: string, options: { from?: string; to?: string; latest?: boolean }) {
  const db = getDb();
  ...
  if (options.latest) {
    ...
    const parseNum = (v: string) => parseInt(v.replace(/^v/, ''), 10);
    fromVersion = db.prepare(
      'SELECT * FROM prompt_versions WHERE prompt_id = ? AND version_number = ?'
    ).get(prompt.id, parseNum(options.from));
    toVersion = db.prepare(
      'SELECT * FROM prompt_versions WHERE prompt_id = ? AND version_number = ?'
    ).get(prompt.id, parseNum(options.to));
