...
// ─── SSE broadcaster ──────────────────────────────────────────────────────────
let sseClients: any[] = [];
export function registerSseClient(res: any) { sseClients.push(res); }
export function unregisterSseClient(res: any) { sseClients = sseClients.filter(c => c !== res); }
...
export type PromptClassification = 'confirmed' | 'candidate' | 'rejected';
...
function pathSlug(projectRoot: string, filePath: string): string {
...
function classify(p: Omit<DetectedPrompt, 'classification' | 'prompt_score' | 'code_score' | 'rejection_reason'>): DetectedPrompt {
...
function deduplicatePrompts(prompts: DetectedPrompt[]): DetectedPrompt[] {
  const byName = new Map<string, DetectedPrompt>();
...
function fileHasLlmCalls(content: string): boolean {
  return LLM_CALL_PATTERNS.some(rx => rx.test(content))
    || WRAPPER_CALL_PATTERNS.some(rx => rx.test(content));
}

function nearLlmCall(content: string, matchIndex: number, windowChars: number = 3000): boolean {
...
export function scanProject(projectRoot: string, config: PromptLogConfig): ScanResult {
  const db = getDb();
  ...
  const result: ScanResult = { confirmed: 0, candidates: 0, rejected: 0, filesScanned: files.length, filesWithPrompts: 0 };
...
export function scanFiles(projectRoot: string, config: PromptLogConfig, filePaths: string[]): ScanResult {
  const db = getDb();
  const gitMeta = getGitMetadata(projectRoot);
...
type RawPrompt = Omit<DetectedPrompt, 'classification' | 'prompt_score' | 'code_score' | 'rejection_reason'>;

function detectPromptsInFile(projectRoot: string, filePath: string, ext: string, content: string): RawPrompt[] {
  ...
  if (instructionPrompts.length > 0) return instructionPrompts;

  if (baseName === '.env' || ext === '.env' || fileLower.endsWith('.env') || fileLower.endsWith('.env.local')) {
    return detectEnvPrompts(slug, content, filePath);
  }

  if (['.yaml', '.yml'].includes(ext)) return detectYamlPrompts(slug, baseName, content, filePath);
  if (ext === '.json') return detectJsonPrompts(slug, baseName, content, filePath, lines);
  if (ext === '.py') return detectPythonPrompts(slug, baseName, content, filePath);
...
function detectInstructionFiles(slug: string, baseName: string, fileLower: string, ext: string, content: string, filePath: string, lines: string[]): RawPrompt[] {
  const isKnown =
    ['claude', 'agents', 'gemini', 'codex'].includes(baseName.toLowerCase()) ||
...
function detectEnvPrompts(slug: string, content: string, filePath: string): RawPrompt[] {
...
function detectYamlPrompts(slug: string, baseName: string, content: string, filePath: string): RawPrompt[] {
  const prompts: RawPrompt[] = [];
...
function detectJsonPrompts(slug: string, baseName: string, content: string, filePath: string, lines: string[]): RawPrompt[] {
  const prompts: RawPrompt[] = [];
...
function detectPythonPrompts(slug: string, baseName: string, content: string, filePath: string): RawPrompt[] {
  const prompts: RawPrompt[] = [];
  ...
  while ((m = pyVarRx.exec(content)) !== null) {
    ...
    if (!isKnown && val.length < 100) continue;
    if (!isKnown && !nearLlmCall(content, m.index)) continue;
  ...
  while ((m = pyFnRx.exec(content)) !== null) {
    const fnName = m[1];
    if (!isPromptBuilderName(fnName)) continue;
    const fnBody = m[2];
...
function detectJsTsPrompts(slug: string, baseName: string, ext: string, content: string, filePath: string): RawPrompt[] {
  const prompts: RawPrompt[] = [];
...
function detectRoleContentObjects(prompts: RawPrompt[], slug: string, lang: string, content: string, filePath: string): void {
  ...
  while ((m = roleRx.exec(content)) !== null) {
    ...
    if (prompts.some(p => p.stable_name === sn)) continue;
    prompts.push({
...
function detectFieldAssignments(prompts: RawPrompt[], slug: string, lang: string, content: string, filePath: string): void {
...
function detectPromptVariables(prompts: RawPrompt[], slug: string, lang: string, content: string, filePath: string, hasLlmCalls: boolean): void {
  ...
  for (const strRx of strPatterns) {
    while ((m = strRx.exec(content)) !== null) {
      const varName = m[1];
      const val = m[2].trim();
      if (val.length < 30 || !isPromptLikeVarName(varName)) continue;
      const sl = lineAt(content, m.index);
      const sn = `${slug}::${varName}`;
      if (prompts.some(p => p.stable_name === sn)) continue;
      prompts.push({
...
function detectPromptRecordObjects(prompts: RawPrompt[], slug: string, lang: string, content: string, filePath: string): void {
  const recordRx = /(?:export\s+)?(?:const|let|var)\s+(\w+)\s*(?::[^=]*)?=\s*\{/g;
  let m: RegExpExecArray | null;
  while ((m = recordRx.exec(content)) !== null) {
    const varName = m[1];
    if (!isPromptLikeVarName(varName)) continue;
...
function detectPromptBuilders(prompts: RawPrompt[], slug: string, lang: string, content: string, filePath: string): void {
...
function detectVercelAiSdk(prompts: RawPrompt[], slug: string, lang: string, content: string, filePath: string): void {
...
function detectMarkdownPrompts(slug: string, baseName: string, fileLower: string, content: string, filePath: string, lines: string[]): RawPrompt[] {
  ...
  if (!isPromptDir && !isPromptNamed) return [];
...
function markRemovedPrompts(seenStableNames: Set<string>): void {
  const db = getDb();
  const projectInfo = db.prepare('SELECT id FROM projects LIMIT 1').get() as { id: string } | undefined;
  if (!projectInfo) return;

  const activePrompts = db.prepare(
    "SELECT id, stable_name, prompt_type FROM prompts WHERE project_id = ? AND status IN ('active', 'candidate')"
  ).all(projectInfo.id) as { id: string; stable_name: string; prompt_type: string }[];

  for (const prompt of activePrompts) {
    if (prompt.prompt_type === 'manual') continue;
...
function trackManualPrompts(projectRoot: string): void {
  const db = getDb();
  const projectInfo = db.prepare('SELECT id FROM projects LIMIT 1').get() as { id: string } | undefined;
  if (!projectInfo) return;

  const manualPrompts = db.prepare(
    "SELECT p.id, p.stable_name, p.current_version_id FROM prompts p WHERE p.project_id = ? AND p.prompt_type = 'manual' AND p.status = 'active'"
  ).all(projectInfo.id) as any[];

  const gitMeta = getGitMetadata(projectRoot);

  for (const prompt of manualPrompts) {
    if (!prompt.current_version_id) continue;
    const version = db.prepare(
      'SELECT source_file, start_line, end_line, content_hash FROM prompt_versions WHERE id = ?'
    ...
    const prevContent = db.prepare(
      'SELECT raw_content FROM prompt_versions WHERE id = ?'
    ).get(prompt.current_version_id) as any;
    ...
    if (currentHash === version.content_hash) continue;

    {
      const versionNum = (db.prepare(
        'SELECT MAX(version_number) as max FROM prompt_versions WHERE prompt_id = ?'
      ).get(prompt.id) as any).max + 1;
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        versionId, prompt.id, versionNum,
        currentContent, currentContent, currentHash,
        version.source_file, actualStart, actualEnd,
        ext, 'manual',
...
function savePromptDetection(
  projectRoot: string,
  prompt: DetectedPrompt,
  gitMeta: ReturnType<typeof getGitMetadata>
): void {
  const db = getDb();
  ...
  let dbPrompt = db.prepare(
    'SELECT id, current_version_id, status FROM prompts WHERE stable_name = ? AND project_id = ?'
  ).get(prompt.stable_name, projectId) as any;
  ...
  if (!dbPrompt) {
    ...
    db.prepare(
      "UPDATE prompts SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
    ).run(dbStatus, dbPrompt.id);
    db.prepare(`
      INSERT INTO prompt_events (id, project_id, prompt_id, event_type, created_by)
      VALUES (?, ?, ?, ?, ?)
  ...
  let currentVersionHash: string | null = null;
  ...
  if (dbPrompt.current_version_id) {
    const v = db.prepare('SELECT version_number, content_hash FROM prompt_versions WHERE id = ?')
      .get(dbPrompt.current_version_id) as any;
    if (v) {
  ...
  if (currentVersionHash !== contentHash) {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      versionId, dbPrompt.id, newVersionNumber,
      prompt.content, normalizedContent, contentHash,
      relPath, prompt.start_line, prompt.end_line,
      prompt.language, prompt.detection_method,
      gitMeta?.branch ?? null,
      gitMeta?.commit ?? null,
      gitMeta?.author ?? null,
      gitMeta?.isDirty ? 1 : 0,
    ...
    db.prepare(
      'UPDATE prompts SET current_version_id = ?, last_seen_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
    ).run(versionId, dbPrompt.id);
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      uuidv4(), dbPrompt.id, versionId, relPath,
      prompt.start_line, prompt.end_line,
      prompt.language, prompt.confidence,
      prompt.symbol_name || null,
      prompt.function_name || null,
    );
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(uuidv4(), projectId, dbPrompt.id, versionId, eventType,
      JSON.stringify({
    ...
    db.prepare(
      'UPDATE prompts SET last_seen_at = CURRENT_TIMESTAMP WHERE id = ?'
    ).run(dbPrompt.id);
  }
}
