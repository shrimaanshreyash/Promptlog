...
export function showStatus(defaultPort) {
    ...
    const total = db.prepare("SELECT COUNT(*) as c FROM prompts WHERE project_id = ?").get(project.id).c;
    const active = db.prepare("SELECT COUNT(*) as c FROM prompts WHERE project_id = ? AND status = 'active'").get(project.id).c;
    const removed = db.prepare("SELECT COUNT(*) as c FROM prompts WHERE project_id = ? AND status = 'removed_from_codebase'").get(project.id).c;
