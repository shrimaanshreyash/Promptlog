...
export function rollback(promptId, versionArg, options) {
    const db = getDb();
    ...
    const targetVersion = db.prepare('SELECT * FROM prompt_versions WHERE prompt_id = ? AND version_number = ?').get(prompt.id, versionNum);
    if (!targetVersion) {
    ...
    if (projectInfo) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(uuidv4(), projectInfo.id, prompt.id, targetVersion.id, 'rollback_patch_created', JSON.stringify({ patchPath: path.relative(projectRoot, patchPath), toVersion: versionNum }), 'cli');
    }
}
