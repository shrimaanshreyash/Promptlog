...
export function addPromptManually(projectRoot, filePath, options) {
    const db = getDb();
    ...
    const existing = db.prepare('SELECT id FROM prompts WHERE stable_name = ?').get(stableName);
    if (existing) {
  ...
  `).run(promptId, projectInfo.id, stableName, `${slug} (${promptName}, L${startLine})`, 'manual', 'active');
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(versionId, promptId, 1, promptContent, promptContent, contentHash, relPath, startLine, endLine, ext, 'manual', gitMeta?.branch ?? null, gitMeta?.commit ?? null, gitMeta?.author ?? null, gitMeta?.isDirty ? 1 : 0, 'active', 'user');
