...
export function getOrComputeDiff(db, promptId, fromVersionId, toVersionId, fromContent, toContent) {
    // Check cache first
    const cached = db.prepare('SELECT * FROM prompt_diffs WHERE from_version_id = ? AND to_version_id = ?').get(fromVersionId, toVersionId);
    if (cached) {
        return {
    ...
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(uuidv4(), promptId, fromVersionId, toVersionId, JSON.stringify(result.lineDiff), JSON.stringify(result.wordDiff), JSON.stringify(result.charDiff), JSON.stringify(result.stats));
    }
    catch {
