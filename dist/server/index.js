...
export function startServer(projectRoot, config, port, host = '127.0.0.1') {
    ...
    app.get('/api/stats', (_req, res) => {
        ...
        const totalPrompts = db.prepare("SELECT COUNT(*) as c FROM prompts WHERE project_id = ?").get(projectInfo.id).c;
        const activePrompts = db.prepare("SELECT COUNT(*) as c FROM prompts WHERE project_id = ? AND status = 'active'").get(projectInfo.id).c;
        const candidatePrompts = db.prepare("SELECT COUNT(*) as c FROM prompts WHERE project_id = ? AND status = 'candidate'").get(projectInfo.id).c;
        const removedPrompts = db.prepare("SELECT COUNT(*) as c FROM prompts WHERE project_id = ? AND status = 'removed_from_codebase'").get(projectInfo.id).c;
        const ignoredPrompts = db.prepare("SELECT COUNT(*) as c FROM prompts WHERE project_id = ? AND status = 'ignored'").get(projectInfo.id).c;
        const totalVersions = db.prepare("SELECT COUNT(*) as c FROM prompt_versions pv JOIN prompts p ON pv.prompt_id = p.id WHERE p.project_id = ?").get(projectInfo.id).c;
    ...
    app.get('/api/prompts/:id/versions', (req, res) => {
        const db = getDb();
        const versions = db.prepare('SELECT * FROM prompt_versions WHERE prompt_id = ? ORDER BY version_number DESC').all(req.params.id);
        res.json({ versions });
    });
    app.get('/api/prompts/:id/versions/:versionId', (req, res) => {
        const db = getDb();
        const version = db.prepare('SELECT * FROM prompt_versions WHERE id = ? AND prompt_id = ?').get(req.params.versionId, req.params.id);
        if (!version)
    ...
    app.get('/api/prompts/:id/diff', (req, res) => {
        ...
        const parseVersionNum = (v) => parseInt(v.replace(/^v/, ''), 10);
        let fromVersion, toVersion;
        if (fromParam && toParam) {
            fromVersion = db.prepare('SELECT * FROM prompt_versions WHERE prompt_id = ? AND version_number = ?').get(req.params.id, parseVersionNum(fromParam));
            toVersion = db.prepare('SELECT * FROM prompt_versions WHERE prompt_id = ? AND version_number = ?').get(req.params.id, parseVersionNum(toParam));
        }
        else {
            // Default: latest vs previous
            const versions = db.prepare('SELECT * FROM prompt_versions WHERE prompt_id = ? ORDER BY version_number DESC LIMIT 2').all(req.params.id);
        ...
        const result = getOrComputeDiff(db, req.params.id, fromVersion.id, toVersion.id, fromVersion.raw_content, toVersion.raw_content);
        res.json({
            diff: result,
    ...
    app.get('/api/prompts/:id/notes', (req, res) => {
        const db = getDb();
        const { version_id, note_type, severity } = req.query;
    ...
    app.post('/api/prompts/:id/versions/:versionId/notes', (req, res) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(noteId, req.params.id, req.params.versionId, note_type || 'general_note', title || '', body || '', severity || 'none', 'user');
        // Log event
        const projectInfo = db.prepare('SELECT id FROM projects LIMIT 1').get();
        if (projectInfo) {
            db.prepare(`
    ...
    app.patch('/api/notes/:noteId', (req, res) => {
        ...
        const existing = db.prepare('SELECT * FROM prompt_notes WHERE id = ?').get(req.params.noteId);
        if (!existing)
      ...
      WHERE id = ?
    `).run(title ?? existing.title, body ?? existing.body, severity ?? existing.severity, note_type ?? existing.note_type, req.params.noteId);
        const projectInfo = db.prepare('SELECT id FROM projects LIMIT 1').get();
        if (projectInfo) {
            db.prepare(`
        INSERT INTO prompt_events (id, project_id, prompt_id, version_id, event_type, event_payload_json, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(uuidv4(), projectInfo.id, existing.prompt_id, existing.version_id, 'note_updated', JSON.stringify({ note_id: req.params.noteId, title: title ?? existing.title }), 'user');
        }
    ...
    app.delete('/api/notes/:noteId', (req, res) => {
        ...
        INSERT INTO prompt_events (id, project_id, prompt_id, version_id, event_type, event_payload_json, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(uuidv4(), projectInfo.id, existing.prompt_id, existing.version_id, 'note_deleted', JSON.stringify({ note_id: req.params.noteId, title: existing.title }), 'user');
        }
    ...
    app.get('/api/notes/search', (req, res) => {
        const db = getDb();
        const { q, note_type, severity, limit: limitParam } = req.query;
    ...
    app.post('/api/notes/bulk-delete', (req, res) => {
        ...
        const notes = db.prepare(`SELECT * FROM prompt_notes WHERE id IN (${placeholders})`).all(...noteIds);
        if (notes.length === 0)
        ...
        if (projectInfo) {
          ...
          INSERT INTO prompt_events (id, project_id, prompt_id, version_id, event_type, event_payload_json, created_by)
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(uuidv4(), projectInfo.id, note.prompt_id, note.version_id, 'note_deleted', JSON.stringify({ note_id: note.id, title: note.title, bulk: true }), 'user');
            }
        }
    ...
    app.post('/api/rollback', (req, res) => {
        ...
        INSERT INTO prompt_events (id, project_id, prompt_id, version_id, event_type, event_payload_json, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(uuidv4(), projectInfo.id, promptId, version.id, 'rollback_patch_created', JSON.stringify({ patchPath: path.relative(projectRoot, patchPath), toVersion }), 'user');
        }
        res.json({
            success: true,
